(ns salesrep.actor
  "SalesRepresentationActor — the ISCO-08 3322 independent sales
  representation actor as a `langgraph.graph/state-graph` (ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one order submission or
  account acceptance operation request (intake → advise → govern → decide →
  ...
  can resume after human sign-off. Modeled on cloud-itonami-isco-3315's
  valuation.actor.
  ...
  The unconditional invariant: the SalesAdvisor can never
  directly submit an order or accept an account the
  SalesRepresentationGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [salesrep.advisor :as advisor]
            [salesrep.governor :as governor]
            [salesrep.store :as store]))

(defn build-graph
  "Build a compiled SalesRepresentationActor graph. `store` implements
  `salesrep.store/Store`. `advisor` implements
  `salesrep.advisor/Advisor` (defaults to `mock-advisor`).
